...
const isBare = (str, allowedProtocols) => !(str.startsWith('/') || str.startsWith('./') || str.startsWith('../') || str.substr(0, 7) === 'http://' || str.substr(0, 8) === 'https://' || allowedProtocols.some((protocol) => str.startsWith(protocol)));

const isString = (str) => typeof str === 'string';

const validate = (map, allowedProtocols) => Object.keys(map.imports).map((key) => {
    const value = map.imports[key];

    if (isBare(value, allowedProtocols)) {
...
export async function load(importMaps = [], allowedProtocols = []) {
    ...
    const mappings = maps.map((item) => {
        ...
        return validate(item, allowedProtocols);
    });
